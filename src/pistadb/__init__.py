...
def _safe_label_bytes(label: str, max_bytes: int = 255) -> bytes:
    """UTF-8 encode + truncate at ``max_bytes`` WITHOUT splitting a codepoint.

    Naive ``label.encode()[:max_bytes]`` can land mid-multibyte-char and
    produce invalid UTF-8 that the native lib mis-renders or rejects.
    """
    if not label:
        return b""
